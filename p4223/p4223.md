...
Date: 2026-05-11  
Repy-to: Ian Petersen \<[ispeters@gmail.com](mailto:ispeters@gmail.com)\>  
Audience: SG1, LEWG
...
[`exec::any_sender`](https://github.com/NVIDIA/stdexec/blob/main/include/exec/any_sender_of.hpp) (and [`unifex::any_sender_of`](https://github.com/facebookexperimental/libunifex/blob/main/include/unifex/any_sender_of.hpp) before it) provides a type-erased container that can store an arbitrary sender that matches the constraints specified by the arguments provided to the class template. For example ([https://godbolt.org/z/W5oYj4a6d](https://godbolt.org/z/W5oYj4a6d)),

// async\_int.h  
\#include \<exec/any\_sender\_of.hpp\>  
\#include \<stdexec/execution.hpp\>

using any\_int\_receiver \= exec::any\_receiver\<  
   STDEXEC::completion\_signatures\<STDEXEC::set\_value\_t(int)\>\>;  
using any\_int\_sender \= exec::any\_sender\<any\_int\_receiver\>;

any\_int\_sender async\_int(int i);

// async\_int.cpp  
\#include "async\_int.h"

namespace ex \= STDEXEC;

any\_int\_sender async\_int(int i) { return ex::just(i); }
...
int main() {  
   auto \[result\] \= ex::sync\_wait(async\_int(42)).value();
...
[`exec::function`](https://github.com/NVIDIA/stdexec/blob/main/include/exec/function.hpp) goes in a different direction; rather than erasing an arbitrary sender, `function` erases a sender *factory* and bundles it with the arguments to that factory. Usage looks like this ([https://godbolt.org/z/Gs1Tb1Tsj](https://godbolt.org/z/Gs1Tb1Tsj)):

// async\_int.h  
\#include \<exec/function.hpp\>  
\#include \<stdexec/execution.hpp\>

exec::function\<int(int) noexcept\> async\_int(int i);
...
Like the existing standard senders, `function` has no required allocation upon construction because it merely curries its arguments into a tuple. When a `function` is connected to a suitable receiver, the curried arguments are passed to the type-erased sender factory resulting in a sender of arbitrary size; that sender is connected to a type-erased receiver that, like with `any_sender`, is of fixed size, producing an operation state of arbitrary size. Thus, `function`, like `any_sender`, may need to dynamically allocate storage for the operation state.
...
* **Composable with other senders**; both solutions solve the problem of separating interface from implementation with a class template, specializations of which model `sender` so both approaches participate fully in the existing asynchronous C++ ecosystem.
...
As Robert Leahy has [explained](https://www.youtube.com/watch?v=S5v7f8rjSVQ), function invocation can be broken down into four steps:
...
// require an inplace\_stop\_token in the receiver's environment  
function\<  
   int(bar, baz),  
   queries\<inplace\_stop\_token(get\_stop\_token\_t) noexcept\>\>

// require that the erased sender be from domain MyDomain  
function\<int(bar, baz), attrs\<MyDomain(get\_domain\_t) noexcept\>\>

// require both  
function\<  
   int(bar, baz),  
   queries\<inplace\_stop\_token(get\_stop\_token\_t) noexcept\>,  
   attrs\<MyDomain(get\_domain\_t) noexcept\>\>

## Controlling Completion Signatures

To erase senders whose completion signatures can’t be described with a function signature, such as those with multiple value or error completions, or completions with more than one parameter in the value channel, the existing `completion_signatures` class template will serve. However, given that `function` erases a sender *factory* and thus must express the factory’s parameter list in its type, we need more than just `completion_signatures`. This paper proposes that the function type syntax communicate the factory parameters, with `sender_tag` as the return type to support future extensions of `function` that can erase senders whose `sender_concept` nested alias refers to future categories of senders. The result looks like this:

// erase a sender that can only complete with set\_value(int)  
function\<  
   sender\_tag(bar, baz),  
   completion\_signatures\<set\_value\_t(int)\>\>

// erase a sender that can fail with an error\_code but not an exception  
function\<  
   sender\_tag(bar, baz),  
   completion\_signatures\<  
       set\_value\_t(int),  
       set\_error\_t(error\_code)\>\>
...
2. The name get\_frame\_allocator denotes a query object. For a subexpression `env`,  
...
3. `forwarding_query(execution::get_frame_allocator)` is a core constant expression and has value `true`.
...
concept *queryable-with-impl* \= // *exposition only*  
   ...
   (\!Nothrow || is\_nothrow\_invocable\_v\<Query, const Env&, Args...\>) &&  
   same\_as\<Return, invoke\_result\_t\<Query, const Env&, Args...\>\>;
...
concept *sender-attributes-contain* \= // *exposition only*  
   *has-query-sigs-v*\<env\_of\_t\<Sndr\>, Queries...\>;
...
struct *opstate* { // *exposition only*  
   using operation\_state\_concept \= operation\_state\_tag;
...
struct *virt-receiver*\<  
   completion\_signatures\<Sigs...\>,  
   Queries...\> { // *exposition only*  
   using receiver\_concept \= receiver\_tag;

   // the set of completion functions specified in Sigs...

   constexpr *virt-env*\<Queries...\> get\_env() const noexcept;  
};
...
inline constexpr bool  
   ...
   *sender-to*\<Sndr,  
             *virt-receiver*\<completion\_signatures\<Sigs...\>, Queries...\>\> &&  
...
concept *matching-sender-factory* \= // *exposition only*  
   ...
   is\_trivially\_copyable\_v\<Factory\> &&  
   *matching-sender-v*\<  
      invoke\_result\_t\<Factory, Args...\>,  
      Sigs,  
      Queries,  
      Attrs\>;
...
struct *func-impl*\<  
   ...
   template \<  
      *matching-sender-factory*\<  
         ...
         Args...\>  
      Factory\>  
       requires(\!same\_as\<*func-impl*, decay\_t\<Factory\>\>)  
   constexpr explicit  
      *func-impl*(Args&&... args, Factory fact) // *exposition only*  
       noexcept((is\_nothrow\_constructible\_v\<decay\_t\<Args\>, Args\> && ...));

   template \<class Sndr, class... Env\>  
       requires(sizeof...(Env) \== 0 && sizeof...(Queries) \== 0) ||  
               (sizeof...(Env) \== 1 && *has-query-sigs-v*\<Env..., Queries...\>)  
   static consteval auto get\_completion\_signatures();

   template \<class Receiver\>  
   *opstate*\<Receiver\> connect(Receiver rcvr) &&;

   template \<class Receiver\>  
       requires copy\_constructible\<\_func\_impl\>  
   *opstate*\<Receiver\> connect(Receiver rcvr) const& {  
       return *func-impl*(\*this).connect(std::move(rcvr));  
   }

  private:  
   *product-type*\<Args...\> *args\_*;  // *exposition only*  
   // chosen to be large enough to store one of:  
   //  \- pointer-to-function  
   //  \- pointer-to-member function  
   //  \- pointer-to-member data  
   static constexpr size\_t *sender-factory-size* \=  
       2 \* sizeof(void\*);  // *exposition only*  
   byte *sender-factory-storage\_*\[*sender-factory-size*\]{}; // exposition  
                                                        // only

   using *virt-receiver-t* \= *virt-receiver*\<completion\_signatures\<Sigs...\>,  
                                         Queries...\>;  // *exposition only*  
   *virt-opstate* (\**operation-factory\_*)(void\*, *virt-receiver-t*,  
...
struct *completions-of-sig-impl*\<Return(Args...), false\> { // *exposition only*  
   using type \=  
       completion\_signatures\<set\_value\_t(Return),  
                             set\_error\_t(exception\_ptr),  
                             set\_stopped\_t()\>;  
};
...
struct *completions-of-sig-impl*\<void(Args...), false\> { // *exposition only*  
   using type \=  
       completion\_signatures\<set\_value\_t(),  
                             set\_error\_t(exception\_ptr),  
                             set\_stopped\_t()\>;  
};
...
struct *completions-of-sig-impl*\<Return(Args...), true\> { // *exposition only*  
   using type \= completion\_signatures\<set\_value\_t(Return),  
                                      set\_stopped\_t()\>;  
};

template \<class... Args\>  
struct *completions-of-sig-impl*\<void(Args...), true\> { // *exposition only*  
   using type \= completion\_signatures\<set\_value\_t(),  
                                      set\_stopped\_t()\>;  
};
...
struct *make-func*\<Return(Args...) noexcept\> { // *exposition only*  
   using type \= *func-impl*\<sender\_tag(Args...),  
...
struct *make-func*\<  
   sender\_tag(Args...),  
   completion\_signatures\<Sigs...\>\> { // *exposition only*  
   using type \= *func-impl*\<sender\_tag(Args...),  
                          completion\_signatures\<Sigs...\>,  
                          queries\<\>, attrs\<\>\>;  
};
...
struct *make-func*\<sender\_tag(Args...), completion\_signatures\<Sigs...\>,  
                 queries\<Queries...\>\> { // *exposition only*  
   using type \= *func-impl*\<sender\_tag(Args...),  
                          completion\_signatures\<Sigs...\>,  
                          queries\<Queries...\>,  
...
1. [P4003R2](https://wg21.link/p4003r2) – “A Minimal Coroutine Execution Model” (Vinnie Falco, Steve Gerbino, Mungo Gill, 2026\)  
2. [P4172R0](https://wg21.link/p4172r0) – “IoAwaitable for Coroutine-Native Byte-Oriented I/O” ((Vinnie Falco, Steve Gerbino, Mungo Gill, 2026\)  
3. [P4127R0](https://isocpp.org/files/papers/P4127R0.pdf) – “The Coroutine Frame Allocator Timing Problem” (Vinnie Falco, C++ Alliance Proposal Team, 2026\)  
4. [https://www.youtube.com/watch?v=S5v7f8rjSVQ](https://www.youtube.com/watch?v=S5v7f8rjSVQ) – “Evolving C++ Networking with Senders & Receivers \[part 1\]” (Robert Leahy, 2024\)

[^1]:  This analogy can also explain the leak of implementation into interface that exists in the standard senders today: today’s fully-concrete senders are like inline functions, where the compiler must be able to see the full implementation to be able to (among other things) allocate space in the caller’s frame for the callee’s activation frame.